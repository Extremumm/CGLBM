...
void print_usage(const std::string& program_name) {
    std::cout << "usage: " << program_name << " [E4|E6|E8] [options]\n"
              ...
              << "                      capillary stress in Omega^(2), a body force from\n"
              << "                      an explicit curvature (Ba et al.), or the same force\n"
              << "                      as the divergence of the capillary stress, which\n"
              << "                      conserves momentum; `csf` with\n"
              << "                      `--interface-field=normalised` is what reaches a\n"
              << "                      density ratio of a few hundred, `stress` with\n"
              << "                      `--viscosity-mixing=dynamic` one of 1e4\n"
              ...
              << "                      interpolate nu, or rho nu, on the volume fraction\n"
              << "                      across the interface; `dynamic` keeps tau uniform\n"
              << "                      there when the dynamic viscosities match\n"
              << "  --lattice=d3q19|d3q27\n"
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (option_value(argument, "surface-tension", &value)) {
            if (value == "perturbation") {
                ...
                config.surface_tension = SurfaceTension::CapillaryStress;
            } else {
                std::cerr << "Unknown surface tension form '" << value
                          << "'; expected perturbation, csf or stress." << std::endl;
                return CommandLineResult::Error;
            }
            continue;
        }
        if (option_value(argument, "viscosity-mixing", &value)) {
            if (value == "kinematic") {
                config.viscosity_mixing = ViscosityMixing::Kinematic;
            } else if (value == "dynamic") {
                config.viscosity_mixing = ViscosityMixing::Dynamic;
            } else {
                std::cerr << "Unknown viscosity mixing '" << value
                          << "'; expected kinematic or dynamic." << std::endl;
                return CommandLineResult::Error;
            }
            continue;
        }
...
std::string describe(const CaseConfig& config) {
    ...
    out << "case = " << config.name << "\n"
        ...
        << (config.surface_tension == SurfaceTension::ContinuumSurfaceForce
                ? "csf"
                : (config.surface_tension == SurfaceTension::CapillaryStress ? "stress"
                                                                             : "perturbation"))
        << "\n"
        << "viscosity_mixing = "
