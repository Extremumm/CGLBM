...
///
/// `CapillaryStress` is the same body force written as the divergence of the
/// capillary stress (see lbm/surface_force.h):
///
///     F_s = div T,   T = sigma/2 (|grad phi_N| I - grad phi_N grad phi_N / |grad phi_N|)
///
/// It equals the continuum-surface-force form in the continuum and also
/// reaches the momentum equation through Guo's forcing, but on the lattice a
/// centred divergence sums to zero over a periodic domain, so the interface
/// exerts no net force on the fluid: it conserves momentum where the
/// curvature form does not. It needs no curvature, only the gradient of
/// `phi_N`, and it is what holds Laplace's law at a density ratio of 1e4
/// together with `ViscosityMixing::Dynamic`. See docs/numerics.md.
///
/// Reference
...
///    J. Comput. Phys. 113, 134 (1994): the capillary stress tensor.
enum class SurfaceTension {
    ...
    CapillaryStress         ///< body force as the divergence of the capillary stress
};

/// How the viscosities of the two components mix across the interface.
///
/// Both rules give `nu_1` and `nu_2` exactly in the two bulks, with
/// `c = (1 + phi_N) / 2` the volume fraction of component 1.
///
/// `Kinematic` interpolates the kinematic viscosity on the volume fraction,
///
///     nu = c nu_1 + (1 - c) nu_2.
///
/// `Dynamic` interpolates the dynamic viscosity instead,
///
///     rho nu = c mu_1 + (1 - c) mu_2,   i.e.  nu = Y_1 nu_1 + Y_2 nu_2
///
/// with `Y_k` the mass fractions `(1 +- phi) / 2` (lbm/mixture.h). The two
/// differ most where the densities do. With equal dynamic viscosities at a
/// density ratio of 1e4, `Kinematic` gives the interface nodes, which already
/// carry most of the heavy fluid's mass, the light fluid's kinematic
/// viscosity, and `tau = rho nu / p + 1/2` reaches about 5e3 there; `Dynamic`
/// keeps `rho nu`, hence `tau`, uniform across the interface.
enum class ViscosityMixing {
    Kinematic,  ///< nu = c nu_1 + (1 - c) nu_2
    Dynamic     ///< rho nu = c mu_1 + (1 - c) mu_2
};
...
struct Physics {
    ...
    /// That is `ViscosityMixing::Kinematic`, the default; `Dynamic` mixes
    /// `rho nu` instead, which is what a large density ratio needs.
    double nu2 = -1.0;
    double nu_b2 = -1.0;
...
struct CaseConfig {
    ...
    /// How two different viscosities mix across the interface. See
    /// `ViscosityMixing`. Ignored when the two components share them.
    ViscosityMixing viscosity_mixing = ViscosityMixing::Kinematic;
