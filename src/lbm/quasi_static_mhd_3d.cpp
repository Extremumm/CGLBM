...
inline void step_along(int axis, int delta, int nx, int ny, int nz, int i, int j, int k, int* out) {
    out[0] = i;
    out[1] = j;
    out[2] = k;
    out[axis] += delta;
        ...
        for (int j = 0; j < ny_; j++) {
            for (int k = 0; k < nz_; k++) {
                for (int axis = 0; axis < 3; axis++) {
                    int n[3];
                    step_along(axis, 1, nx, ny, nz, i, j, k, n);
                    const double face = 0.5 * (emf_(i, j, k, axis) + emf_(n[0], n[1], n[2], axis));
            ...
            for (int j = 0; j < ny_; j++) {
                for (int k = 0; k < nz_; k++) {
                    ...
                    const double omega_plus =
                        slack > 0.0 ? 1.0 / (kMagicParameter / slack + 0.5) : 0.0;
