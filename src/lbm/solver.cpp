...
Solver::Solver(CaseConfig config) : config_(std::move(config)), nx_(config_.nx), ny_(config_.ny) {
    ...
    dynamic_mixing_ = config_.viscosity_mixing == ViscosityMixing::Dynamic;
    ...
    if (config_.surface_tension == SurfaceTension::CapillaryStress) {
        stress_xx_ = Field(nx_, ny_);
        stress_xy_ = Field(nx_, ny_);
        stress_yy_ = Field(nx_, ny_);
    }
...
void Solver::viscosity_at(int i, int j, double* nu, double* nu_b) const {
    ...
    if (dynamic_mixing_) {
        // rho nu = c mu_1 + (1 - c) mu_2, which is nu weighted by the mass
        // fractions (1 +- phi) / 2 -- the colour field itself.
        *nu = mixture_kinematic_viscosity(phi_(i, j), config_.physics.nu, nu2_);
        *nu_b = mixture_kinematic_viscosity(phi_(i, j), config_.physics.nu_b, nu_b2_);
        return;
    }
...
#pragma omp parallel for collapse(2) if (parallel)
    for (int i = 0; i < nx_; i++) {
        for (int j = 0; j < ny_; j++) {
            ...
            // it is zero unless the case applies the tension as a body force.
...
void Solver::capillary_stress_force() {
    ...
    // Pass one: the stress sigma/2 (|g| I - g g / |g|), g = grad phi_N, which
    // is what the colour gradient holds when the interface field is phi_N.
#pragma omp parallel for collapse(2) if (parallel)
    for (int i = 0; i < nx_; i++) {
        for (int j = 0; j < ny_; j++) {
            ::cglbm::lbm::capillary_stress(sigma,
                                           grad_phi_(i, j, 0),
                                           grad_phi_(i, j, 1),
                                           &stress_xx_(i, j),
                                           &stress_xy_(i, j),
    ...
    // Pass two: F_s = div T, on the colour gradient's own stencil. A centred
    // divergence sums to zero over a periodic lattice, so the interface
    // exerts no net force on the fluid.
#pragma omp parallel for collapse(2) if (parallel)
    for (int i = 0; i < nx_; i++) {
        for (int j = 0; j < ny_; j++) {
            ::cglbm::lbm::surface_force(stress_xx_.data(),
                                        stress_xy_.data(),
                                        stress_yy_.data(),
                                        nx_,
                                        ny_,
                                        ...
                                        stencil,
                                        boundary,
                                        &force_surface_(i, j, 0),
                                        &force_surface_(i, j, 1));
        }
    }
}

void Solver::collide_surface() {
    if (config_.surface_tension != SurfaceTension::Perturbation) {
