...
struct LatticeUnits {
    ...
    double cs() const { return dx / std::sqrt(3.0) / dt; }
    double cs2() const { const double c = cs(); return c * c; }
    double cs4() const { const double c2 = cs2(); return c2 * c2; }
    double cs6() const { return cs4() * cs2(); }
};
