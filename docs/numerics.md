...
| $\phi$ | `phi_` | phase field, $\phi = +1$ in fluid 1, $-1$ in fluid 2; a **mass**-fraction difference $Y_1 - Y_2$ |
| $\phi_N$ | `phi_n_` | the bulk-normalised phase field, the **volume**-fraction difference $2c - 1$ |
...
1. **$\Omega^{(1)}$ — regularized relaxation** (`collide`). The non-equilibrium
   ...
   pressure from the equation of state, not on $\rho c_s^2$. Each component
   can have its own viscosity, mixed across the interface as ν or as ρν
   (`ViscosityMixing`); see
...
2. **$\Omega^{(2)}$ — surface tension** (`collide_surface`). A perturbation
   ...
   $\sigma$ and reproduces the Laplace jump. `SurfaceTension::ContinuumSurfaceForce`
   and `SurfaceTension::CapillaryStress` replace it by a body force on
   $\nabla\phi_N$, which enters through `S_F` (`surface_force`).
3. **$\Omega^{(3)}$ — recoloring** (`recolor`). Redistributes the components
   ...
   thickness (`ch_width_init` is used only when initialising). With
   `InterfaceField::BulkNormalised` the direction is the normal of $\phi_N$
   rather than of $\phi$.
...
- `S_F` — the body force: gravity `a_g` in the gravity cases, the surface force
  when it is applied as one, zero otherwise;
...
A case chooses between three ways of applying the surface tension, and only one
...
with gravity; `SurfaceTension::CapillaryStress` adds the same body force as the
divergence of the capillary stress, which conserves momentum. See
[How far the density ratio goes](#how-far-the-density-ratio-goes) and
[Beyond 500](#beyond-500-the-viscosity-mixing-and-the-capillary-stress).
...
τ is 5.49 in the two bulks there, not across the interface: interpolating ν on
the volume fraction puts τ ≈ 1.2 × 10⁴ on the half-volume node at 10⁴.
`ViscosityMixing::Dynamic` interpolates ρν instead, which keeps τ uniform
everywhere, and that does rescue it; see
[The viscosity has to be mixed as ρν](#the-viscosity-has-to-be-mixed-as-ρν).
...
### The two interfaces separate: φ is a mass fraction

The radius above is the one the **density** field settles at. It has to be
stated, because under the restored equation of state, unlike the linear mixing
rule, the phase field and the density field come to rest at different radii.
...
Nothing in the scheme moves it, and the equation of state predicts it.
φ = (ρ₁ − ρ₂)/(ρ₁ + ρ₂) is built on the partial densities carried by f and g,
so Y₁ = (1 + φ)/2 is a mass fraction. The equation of state is the
pressure-equilibrium mixture 1/ρ = Y₁/ρ₁(p) + Y₂/ρ₂(p), and at fixed pressure
the density is linear in the *volume* fraction α₁ = ρY₁/ρ₁(p), not in φ. The
two fractions differ by a logistic shift,
$\operatorname{logit} Y_1 = \operatorname{logit}\alpha_1 + \ln(\rho_1/\rho_2)$,
so the tanh profile of width W that the recolouring maintains in φ is the same
profile in α₁, displaced by

$$\delta = \tfrac{W}{2}\ln\frac{\rho_1}{\rho_2}.$$

With W = 1.6 that is 2.40 lattice units at a density ratio of 20, against the
2.35 measured, and 7.37 at 10⁴ (`src/lbm/mixture.h`, checked by
`programs/unit_testing/lbm/mixture`). The linear mixing rule had no gap because
it made φ a volume fraction.
...
`programs/solvers/color_gradient/laplace/tests/` checks the gap against δ, so
that a scheme change moving it is visible. The equilibrium is not where it comes
from: the enhanced equilibrium of Leclaire et al. (2013) and the third-order term
...
## Beyond 500: the viscosity mixing and the capillary stress

[How far the density ratio goes](#how-far-the-density-ratio-goes) stops
`Solver` at a density ratio of about 500, and
[Trying for 10⁴](#trying-for-10-and-how-the-earlier-claims-went-wrong) finds
10³ and 10⁴ out of reach with matched dynamic viscosities. Two options of
`Solver` change that. They come from a separate line of work on the earlier,
...
That is the shipped case with a droplet 10⁴ times denser than its surroundings
and the same dynamic viscosity in both fluids; everything else, including the
equilibrium start and the φ_N interface, is the shipped configuration.

### The viscosity has to be mixed as ρν

Matching the dynamic viscosities in the two bulks, `nu2 = nu rho1/rho2`, makes τ
the same in both, but `ViscosityMixing::Kinematic` then interpolates ν on the
volume fraction across the interface. At the half-volume node of a 10⁴ droplet
that is ρ ≈ ρ₁/2 against ν ≈ ν₂/2, and τ = ρν/(p dt) + ½ ≈ 1.2 × 10⁴: the span
of τ that matching was meant to remove is still there, on the interface nodes,
which is where the tension acts. That is why matching "does not by itself
rescue the density ratio" above.

`ViscosityMixing::Dynamic` interpolates the dynamic viscosity instead,

$$\rho\nu = c\,\mu_1 + (1 - c)\,\mu_2,\qquad\text{i.e.}\qquad \nu = Y_1\nu_1 + Y_2\nu_2,$$

with $Y_k = (1 \pm \phi)/2$ the mass fractions (`mixture_kinematic_viscosity`,
`src/lbm/mixture.h`). With μ₁ = μ₂ it gives τ = 5.5 at every node. Measured at
10⁴, with either force below, after 10⁴ steps (CSF) and 9 × 10³ (stress):

| | Δp / (σ/R(ρ)), CSF | Δp / (σ/R(ρ)), stress | max &#124;u&#124;, CSF and stress |
|---|---|---|---|
| kinematic mixing | 1.137 | 1.172 | 1.1 × 10⁻² and 4.4 × 10⁻², rising |
| dynamic mixing | 1.016 | 1.019 | 2.0 × 10⁻³ and 2.1 × 10⁻³, falling |

With one kinematic viscosity for both fluids, as the shipped case has, τ
reaches 5 × 10⁴ in the droplet at 10⁴ whichever rule mixes it; that case is
out of reach for this reason alone.

### The capillary stress

`SurfaceTension::CapillaryStress` applies the tension as the divergence of the
capillary stress tensor of Lafaurie et al. (1994),

$$\vec F = \nabla\cdot\mathsf T,\qquad \mathsf T = \frac{\sigma}{2}\left(\lvert\nabla\phi_N\rvert\,\mathsf I - \frac{\nabla\phi_N\,\nabla\phi_N}{\lvert\nabla\phi_N\rvert}\right),$$

through the same Guo forcing as `ContinuumSurfaceForce`
(`src/lbm/surface_force.h`). In the continuum the two are the same force,
$\tfrac{\sigma}{2}\kappa\nabla\phi_N$, and they give the same static jumps. On
the lattice only the divergence form conserves momentum: a centred divergence
sums to zero over a periodic lattice whatever the shape of the interface. The
unit test of `Solver` checks that on a tilted ellipse centred between nodes,
where no symmetry cancels a net force: after 20 steps the stress form leaves the
fluid with a total momentum of 10⁻¹⁴ to 5 × 10⁻¹⁴, the curvature form 7 × 10⁻⁴ to
1.5 × 10⁻³. On a droplet translating at 0.01 lattice units per step at a density
ratio of 100, the curvature form drained 36 % of the total momentum in 3000
steps in the earlier program, where the stress form conserved it to the last
digit. It needs no curvature, only the gradient `recolor()` already uses.

### Results

The Laplace case with matched dynamic viscosities, `--viscosity-mixing=dynamic`,
3 × 10⁴ steps (5.5 × 10³ relaxation times at τ = 5.5), scored against the radius
the density settles at:

| ρ₁/ρ₂ | force | Δp / (σ/R(ρ)) | R(ρ) | R(φ = 0) − R(ρ) | max &#124;u&#124; |
|---|---|---|---|---|---|
| 10³ | CSF | 1.024 | 10.02 | 5.01 | 2.0 × 10⁻⁵ |
| 10³ | stress | 1.027 | 10.02 | 5.01 | 2.0 × 10⁻⁵ |
| 10⁴ | CSF | 1.015 | 9.98 | 6.52 | 6.4 × 10⁻⁴ |
| 10⁴ | stress | 1.018 | 9.98 | 6.52 | 6.5 × 10⁻⁴ |
...
- **Moving interfaces at high density ratio.** In the earlier program, a
  ...
  within 15 steps. None of these helped: making the colour transport monotone
  (a per-link limiter that is exact where both populations are positive), a
  wider interface, accelerating the droplet smoothly instead of starting it at
...
| | Δp / (σ/R) | max &#124;u&#124; |
|---|---|---|
| colour-gradient `laplace`, dynamic mixing, stress (30 000 steps) | 1.018 | 6.5 × 10⁻⁴ |
