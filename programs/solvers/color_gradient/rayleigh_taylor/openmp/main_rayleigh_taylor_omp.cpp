...
// const int Ly = 1024; // Number of lattice nodes in the y-direction
const int Lx = 1024; // Number of lattice nodes in the x-direction
const int Ly = 4096; // Number of lattice nodes in the y-direction
...
const int numSteps = 2000000; // Number of simulation steps
...
// (e.g. `rayleigh_taylor_omp E6`). See src/lbm/isotropic_gradient.h.
//
...
// law hold to O(10^4).
//
// E4 is the default here because this case has walls on y: it reaches a single
// node and so reproduces the original scheme exactly, while a wider stencil
// becomes one-sided over two nodes near the wall and has not been validated
// against this case.
cglbm::lbm::GradientStencil gradient_stencil = cglbm::lbm::GradientStencil::E4;

void calEquilibrium() {
    #pragma omp parallel for collapse(2) // Parallelize over both i and j loops
    for (int i = 0; i < Lx; i++) {
        for (int j = 0; j < Ly; j++) {
...
void calPhaseField() {
    ...
    for (int i = 0; i < Lx; i++) {
        for (int j = 0; j < Ly; j++) {
            ...
            //
            // A linear mixing rule used to overwrite this value. On the Laplace
            // case that cost 17 % of the pressure jump at a density ratio of 10 and
            // 28 % at 20; it is kept as pressure_linear_mixing() for comparison.
...
void collide_surface(){
    ...
    for (int i=0 ; i<Lx ; i++){
        for (int j=0 ; j<Ly ; j++){
            ...
            // Colour gradient, already carrying the 1/c_s^2 factor. Terms that
            // would cross a y wall are dropped, as they were inline here.
...
#include <sstream>  // For string stream to buffer the output
...
void initialize() {
    ...
    int x0 = Lx / 2;
    int y0 = Ly / 2;
    double c0 = 0.1; // ampltitude de perturbation
    ...
    for (int i=0; i<Lx ; i++){
        for (int j=0; j<Ly ; j++){
            ...
            double phi_local = tanh((j-y0-c0*Lx*cos(-2*M_PI*i/Lx))/ch_width_ope); // cherche k la plus instable (formule) avec les paramètres de la simulation
