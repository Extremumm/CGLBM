...
const int numSteps = 50000; // Number of simulation steps
...
const double a_g = 9.81/(c_dx/c_dt/c_dt); // m/s^2
//parameters
...
const double sigma = 0.02/(c_dx * c_dx * c_dx / c_dt / c_dt); //1.e-3 / (347*347*3*1.e-4);// 1e-3; //s1.e-3; // surface tension
...
// (e.g. `gravity_capillary E6`). See src/lbm/isotropic_gradient.h.
//
...
void runSimulation() {
    ...
    outputDataCSV(0);
    std::ofstream fileInterface;
    fileInterface.open("interface.csv");
    fileInterface << "Timestep, phi, y" << std::endl;
