...
**What is asserted, and what is only recorded.** The frequency comes out 13 %
below Lamb's at the shipped resolution, and that gap is the finding rather than
a failure: it shrinks as ``1 - 1.3/R`` as the droplet is better resolved (0.834
at R = 8, 0.872 at R = 10, 0.894 at R = 13), while the static surface tension at
these exact parameters is right to 2.4 % and the damping accounts for under
1 %. So the tests below assert the measured value with a tolerance that would
catch a change in the scheme, and assert separately the things that must be
exactly true whatever the resolution: that the two equatorial semi-axes stay
equal, that the droplet keeps its volume, and that what is being fitted really
is a single decaying mode.
"""
...
MEASURED_LAMB_RATIO = 0.872

#: Measured period, in steps. Lamb's is 725.5 for the same numbers.
MEASURED_PERIOD = 839.0

#: Measured `alpha / omega_0`. Small enough that the correction from the
#: observed frequency to the undamped one is 0.8 %.
MEASURED_DAMPING_RATIO = 0.123
...
#: pressure compresses it, exactly as in `laplace_3d`, which relaxes to 9.69.
MEASURED_MEAN_RADIUS = 9.70
...
def test_verification_oscillation_3d_rings_down_as_a_single_mode(track, fit):
    ...
    """
    assert fit.residual < 0.05 * track["signal"][SETTLING_STEPS:].std()
...
def test_validation_oscillation_3d_frequency_against_lamb(run_3d, track, fit):
    ...
    surface tension was measured at in `laplace_3d`. The ratio is 0.872 and the
    tolerance is set to catch a change in the scheme rather than to declare
    agreement: what closes the remaining gap is resolution, and the module
    docstring has the three radii that show it.
    """
    lamb = lamb_frequency(
        mode=2,
...
def test_validation_oscillation_3d_is_far_from_a_standing_still_droplet(track, fit):
    ...
    `laplace_3d` leaves spurious currents at 2e-5 in a droplet that is not
    ...
    assert track["signal"].max() - track["signal"].min() > 2.0
