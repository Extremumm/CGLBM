...
@pytest.mark.parametrize(
    "boundary",
    ["periodic", "wall", "csf", "latva_kokko", "viscosity_ratio", "stress", "dynamic_mixing"],
)
def test_unit_test_lbm_solver_conserves_mass(solver_report, stencil, boundary):
    ...
    different kinematic viscosities. `stress` applies the tension as the
    divergence of the capillary stress, and `dynamic_mixing` mixes the two
    viscosities as rho nu. None shares a code path with the two above, so all
    are checked against the same invariants.
    """
...
@pytest.mark.parametrize("stencil", STENCILS)
@pytest.mark.parametrize(
    "boundary", ["periodic", "wall", "csf", "viscosity_ratio", "stress", "dynamic_mixing"]
)
...
#: Total momentum a still fluid may pick up from its own interface, relative to
#: its mass. The stress form measures 1e-14 to 5e-14 on a mass of 3.0e3, i.e.
#: rounding; the curvature form 7e-4 to 1.5e-3.
NET_FORCE_TOLERANCE = 1.0e-15


@pytest.mark.unit_test
@pytest.mark.parametrize("stencil", STENCILS)
def test_unit_test_lbm_solver_capillary_stress_exerts_no_net_force(solver_report, stencil):
    """The divergence of a stress sums to zero over a periodic lattice.

    A tilted ellipse centred between nodes has no symmetry that would cancel a
    net surface force on its own, so a still fluid around it keeps zero total
    momentum only if the force is a discrete divergence. The curvature form is
    checked alongside to show the test tells the two apart.
    """
    values = solver_report[stencil]
    mass = float(values["net_force_stress_mass"])
    stress = float(values["net_force_stress_max_total_momentum"])
    csf = float(values["net_force_csf_max_total_momentum"])
    assert stress / mass < NET_FORCE_TOLERANCE
    assert csf / mass > 1.0e3 * NET_FORCE_TOLERANCE
