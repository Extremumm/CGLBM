...
/// The two options that carry the Laplace case to a density ratio of 1e4: the
/// tension as the divergence of the capillary stress, and the viscosities
/// mixed as rho nu. Each takes a branch no other case here does.
CaseConfig stress_case(GradientStencil stencil) {
    CaseConfig config = ba_case(stencil);
    config.surface_tension = cglbm::lbm::SurfaceTension::CapillaryStress;
    return config;
}

CaseConfig dynamic_mixing_case(GradientStencil stencil) {
    CaseConfig config = viscosity_ratio_case(stencil);
    config.surface_tension = cglbm::lbm::SurfaceTension::CapillaryStress;
    config.viscosity_mixing = cglbm::lbm::ViscosityMixing::Dynamic;
    return config;
}

/// Total momentum sum(rho u) over the lattice, x and y.
void total_momentum(const Solver& solver, double* px, double* py) {
    const cglbm::lbm::Field& rho = solver.density();
    const cglbm::lbm::Field& u = solver.velocity();
    *px = 0.0;
    *py = 0.0;
...
/// The net force the interface exerts, read off the momentum of a still fluid.
///
/// A droplet centred on a lattice node is symmetric, and any surface force sums
/// to zero over it by symmetry alone. This one is a tilted ellipse centred
/// between nodes, so only a force written as the divergence of a stress --
/// whose centred differences sum to zero over a periodic lattice whatever the
/// shape -- leaves the total momentum at zero. The curvature form is reported
/// beside it for comparison.
void report_net_surface_force(const std::string& prefix,
                              GradientStencil stencil,
                              cglbm::lbm::SurfaceTension tension) {
    CaseConfig config = ba_case(stencil);
    config.surface_tension = tension;
    config.initial_phase = [](const CaseConfig& c, int i, int j) {
        const double angle = 0.5;
        const double dx = i - 13.3;
        const double dy = j - 17.7;
        const double along = (std::cos(angle) * dx + std::sin(angle) * dy) / 7.0;
        const double across = (-std::sin(angle) * dx + std::cos(angle) * dy) / 4.5;
        const double scaled = std::sqrt(along * along + across * across);
        return -std::tanh((scaled - 1.0) * 5.6 / c.physics.ch_width_init);
    };
    ...
    for (int step = 0; step < config.steps; ++step) {
        ...
        total_momentum(solver, &px, &py);
        worst = std::max(worst, std::hypot(px, py));
    }
    std::cout.precision(17);
    std::cout << prefix << "_max_total_momentum = " << worst << "\n"
              << prefix << "_mass = " << total_mass(solver) << std::endl;
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        report_case("stress", stress_case(stencil));
        report_case("dynamic_mixing", dynamic_mixing_case(stencil));
        report_net_surface_force(
            "net_force_stress", stencil, cglbm::lbm::SurfaceTension::CapillaryStress);
        report_net_surface_force(
            "net_force_csf", stencil, cglbm::lbm::SurfaceTension::ContinuumSurfaceForce);
