...
void report_positivity_bound() {
    ...
    for (Lattice3DKind kind : kLattices) {
        ...
        for (int q = 1; q < lattice.q; ++q) {
            ...
            const double phi = cglbm::lbm::rest_weight_3d(lattice, q, alpha);
            bound = std::min(bound,
                             phi / (3.0 * lattice.w[q] * std::sqrt(e_squared) * std::fabs(factor)));
        }
        const double predicted = kind == Lattice3DKind::D3Q27 ? cs_squared / 2.0 : cs_squared / 3.0;
...
void report_high_ratio_run() {
    ...
    for (Lattice3DKind kind : kLattices) {
        ...
        std::cout << tag
                  << "_mass1_drift = " << std::fabs(component_mass(solver, 0) - mass1) / mass1
                  << "\n"
                  << tag
                  << "_mass2_drift = " << std::fabs(component_mass(solver, 1) - mass2) / mass2
                  << "\n"
